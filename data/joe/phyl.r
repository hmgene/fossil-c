...
library(tools)
input="bwa_scores/"
tsv_files <- list.files(input, pattern = "*\\.tsv$", full.names = TRUE, recursive = F)
#tsv_files <- list.files(input, pattern = "Trex_(cells|vessels).*\\.tsv$", full.names = TRUE, recursive = F)
#tsv_files <- list.files(input, pattern = "Trex_cells.*\\.tsv$", full.names = TRUE, recursive = F)
...
for (grp in names(groups)) {
  ...
  for (tsv in groups[[grp]]) {
    ...
    fwrite(dt_best,file=paste0("bigdata/",sample_name,".csv.gz")) ## bigdata
    library(ggplot2)
    library(dplyr)
...
dt <- dt_best %>%
  ...
  summarise(
    n = n(),
    dens = if (n >= 2) list(density(best_score / read_len, adjust = 1.2)) else list(NULL),
    .groups = "drop"
  ) %>%
  mutate(
    x = lapply(dens, function(d) if (is.null(d)) NA else d$x),
    y = lapply(dens, function(d) if (is.null(d)) NA else d$y)
  ) %>%
  select(ref_label, x, y) %>%
  unnest(cols = c(x, y))
    ...
    dt_norm <- dt_best %>% group_by(ref_label) %>%
      summarise( n=n(), dens =if(n>=2) list(density(best_score / read_len, adjust = 1.2)) else list(NULL), .groups = "drop") %>% mutate( x = lapply(dens, function(d) d$x), y = lapply(dens, function(d) d$y / max(d$y))) %>%
