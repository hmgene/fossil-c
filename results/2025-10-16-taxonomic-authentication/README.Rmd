...
for (grp in names(groups)) {
  ...
  for (tsv in groups[[grp]]) {
    ...
    p <- ggplot(dt_best_filtered, aes(x = best_score, fill = best_ref)) +
      ...
      ggtitle(paste0(sample_name, " | Total: ", total_N, " | >20: ", short_N)) +
      xlab("Best Score") + ylab("Count") + theme_bw(base_size = 9) + theme(legend.position = "none")
    plots[[sample_name]] <- p
  ...
  ggsave(filename = paste0("../../bigdata/bwa_scores/group_", grp, "_grid.png"),
         plot = final_plot, width = 5 * length(plots) + 2, height = 5)
  md_save_pdf(final_plot,paste0("group_", grp, "_grid.png"), width = 5* length(plots)+2, height = 5);
  ...
  combined_row <- plot_grid(plotlist = plots, ncol = length(plots))
  final_plot <- plot_grid(combined_row, legend, ncol = 2, rel_widths = c(1, 0.15))
  ggsave(filename = paste0("../../bigdata/bwa_scores/group_", grp, "_lt60_grid.png"),
         plot = final_plot, width = 5 * length(plots) + 2, height = 5)
  md_save_pdf(final_plot,paste0("group_", grp, "_lt60_grid.png"), width = 5* length(plots)+2, height = 5);
}
```
